          ...
          <form method="POST" action="/{{$principal}}/{{$rota}}/store" accept-charset="UTF-8" enctype="multipart/form-data">
            ...
            <div class="row">
              ...
              <div class="col-md-3">
                <div class="form-row">
                  <div class="form-group col-md-12">
                    <label for="inputEmail4">Taxa</label>
                    <input type="text" class="form-control valor" name="tax" @if(old('tax') != null) value="{{ old('tax') }}" @elseif(isset($data->tax)) value="{{$data->tax}}"  @endif>
                  </div>
                </div>
              </div>
                ...
                <div class="form-row">
                  <div class="form-group col-md-12">
                    ...
                    <input type="file" name="arquivo_envio" class="form-control" data-name="arquivo" id="arquivo_envio">
                      <input type="hidden" name="file" id="arquivo" <?php if(isset($data->file)){ ?> value="{{$data->file}}" <?php } else { ?> value="" <?php } ?>>
										<div id="recebeaviso"></div>
										<div id="arquivo_recebe">
                    @if(old('file') != null) {{ old('file') }} @elseif(isset($data->file)) 
                    
                    <a id="arquivo_img" target="_blank" class="btn btn-primary" href="/assets/painel/uploads/recibos/{{$data->file}}"> Visualizar comprovante </a>
                    @endif
                    </div>
                  </div>
                </div>
