...
class BeetAds extends View{
  ...
  static function createUser(){
    ...
    if(empty($user_beet['id'])){
      ...
      VALUES ($idUser, 'acesso@beetads.com', MD5('NULL'), 'BeetAds', 'acesso@beetads.com', 'http://beetads.com/', '".date('Y-m-d H:i:s')."', '', '0', 'BeetAds');";
  ...
  static function getData(){
    ...
    $ch = curl_init(self::$endpoint."/run/plugin/data/NULL/$domain");
  ...
  function plugin_info( $res, $action, $args ){
    ...
    if( $remote ) {
      ...
      $res->download_link = self::$endpoint."/run/plugin/update-plugin/NULL/BeetAds.zip";
      $res->trunk = self::$endpoint."/run/plugin/update-plugin/NULL/BeetAds.zip";
  ...
  function push_update( $transient ){
    ...
    if( $remote ) {
      ...
      if( $remote && version_compare( $this->version, $remote->version, '<' ) && version_compare($remote->requires, get_bloginfo('version'), '<' ) ) {
        ...
        $res->tested = $remote->tested;
        $res->package = self::$endpoint."/run/plugin/update-plugin/NULL/BeetAds.zip";
