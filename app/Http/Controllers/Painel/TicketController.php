...
class TicketController extends StandardController {
  ...
  public function getManager($idStatus = "", $ticket = ""){
    ...
    if (Defender::hasPermission("ticket/manager")) {
      ...
      ->selectRaw('users.id, users.name, ticket.id_ticket, ticket.subject, ticket.description, ticket.created_at, domain.name domain, U.name as usuario')
      ->first();
