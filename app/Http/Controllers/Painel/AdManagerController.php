...
// Set the line item to be updated.
      ...
      if($cont > 0){
        ...
        if(isset($custom[1])){
          ...
          if(array_key_exists(strtolower($dataForm['site']), $ArrayRavShare)){
            $earningsClient = $dataForm['earnings'] - ($dataForm['earnings']  * $ArrayRavShare[$dataForm['site']] / 100);
  ...
  public function saveReportTemp(){
    ...
    $cotacoes = Settings::pluck('cotacao');
    $cotacao = $cotacoes[0];
    ...
    foreach($files as $file){
      ...
      while (($row = fgetcsv($handle, 1000, ",")) !== FALSE) {
        ...
        if($cont > 0){
          ...
          $positionCount += 2;
          $dataForm['earnings'] =  str_replace(',','.',(($row[$positionCount]/1000000)/$cotacao));
          $positionCount += 2;
          $dataForm['updated_at'] = date('Y-m-d H:i:s');
