...
class UsersController extends StandardController {
  ...
  public function getClients($id=null) {
    ...
    if (Defender::hasPermission("users/clients")) {
      ...
      if($id){
        ...
        ->selectRAW('users.email,users.CPF_CNPJ, users.id, users.name, users.password, users.company, users.created_at, users.updated_at, contract_user.signature, users.status_admanager, 	users.whatsapp, users.disapproved, users.status_waiting, users.invite_admanager')
        ->where('id', $id)
        ->paginate(1000);
        ...
        ->selectRAW('users.email,users.CPF_CNPJ, users.id, users.name, users.password, users.company, users.created_at, users.updated_at,  users.status_admanager, 	users.whatsapp, users.disapproved, users.status_waiting, users.invite_admanager, GROUP_CONCAT(D.name) as dominios')
  ...
  public function getShow($id) {
    ...
    if (Defender::hasPermission("users/lead") || Defender::hasPermission("users") || Defender::hasPermission("users/clients")) {
      ...
      return view("{$this->diretorioPrincipal}.{$this->nameView}.create-edit", compact('data','domains','UserTypes','UserTypeSelected', 'titulo', 'principal', 'rota', 'primaryKey', 'Grupos', 'GrupoSelecionado','bancos','tickets','currencies','finans', 'adx','adx_report'));
    } else {
      return redirect("/{$this->diretorioPrincipal}");
    }
  }
  ...
  public function RegisterSite(Request $request){
    ...
    if(empty($check->id)){
      ...
      Mail::send('emails.boas-vindas',[], function ($m) use($user){
        $m->from("contato@monetizerads.com", "Monetizer Ads");
        $m->to($user->email)->subject($user->name.', seja bem-vindo a MonetizerAds!');
      });
  ...
  public function husky_create(){
    ...
    $cliente_id = "f297844afb331600fa29c41d90229fc71842db3f2492c8edcd96d2ec4784bfc1";
    $secret_id  = "235fcd3caa3c294b0f214429345f6158ebaec572c2c4b852923336d8ab19fed8";
